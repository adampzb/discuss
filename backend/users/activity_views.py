...
class UserActivityStatsView(generics.GenericAPIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        activities_by_type_dict = {
            item["activity_type"]: item["count"] for item in activities_by_type
        }
...
class UserSessionDetailView(generics.RetrieveDestroyAPIView):
    ...
    def delete(self, request, *args, **kwargs):
        ...
        return Response(
            {"detail": "Session ended successfully."}, status=status.HTTP_200_OK
        )
    ...
    def get(self, request, *args, **kwargs):
        ...
        metrics = {
            ...
            "this_week": {
                "start": this_week,
                "count": UserActivity.objects.filter(
                    user=user, created_at__gte=this_week
                ).count(),
            },
            "this_month": {
                "start": this_month,
                "count": UserActivity.objects.filter(
                    user=user, created_at__gte=this_month
                ).count(),
            },
            "last_30_days": {
                "start": today - timedelta(days=30),
                "count": UserActivity.objects.filter(
                    user=user, created_at__gte=today - timedelta(days=30)
                ).count(),
            },
        }
